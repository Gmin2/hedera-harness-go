...
func (t *toolItem) body(w int) string {
	...
	out := strings.TrimRight(cleanText(t.result.Output), " \n")
	...
	style := t.sty.Run.Output
	if t.result.IsError {
		style = t.sty.Run.BodyBad
	}
	...
	if hidden > 0 {
		rows = append(rows, bodyLine(w, t.sty.Subtle.Render(fmt.Sprintf("… (%s hidden)", plural(hidden, "line")))))
	}
...
func (j *judgeSummaryItem) Render(width int) string {
	...
	for _, finding := range f.Findings {
		wrapped := ansi.Wrap(strings.TrimSpace(cleanText(finding)), max(1, w-hang), "")
